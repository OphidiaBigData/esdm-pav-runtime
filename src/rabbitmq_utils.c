...
int set_rabbitmq_connection(amqp_connection_state_t ** conn, amqp_channel_t channel, char *hostname, char *port, char *username, char *password, char *queue_name, amqp_bytes_t * queuename)
{
	...
	if (!hostname || !port || !username || !password || !queue_name) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Failed to open RabbitMQ connection. Wrong parameters\n");
	...
	**conn = amqp_new_connection();
	...
	while (amqp_socket_open(socket, hostname, atoi(port)) != AMQP_STATUS_OK) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Failed to open TCP socket, is rabbitmq server running? I'll try to connect shortly\n");
		sleep(5);
	}
	...
	if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot login - %s\n", amqp_error_string2(reply.library_error));
		return RABBITMQ_FAILURE;
	}
	...
	if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot open channel - %s\n", amqp_error_string2(reply.library_error));
		return RABBITMQ_FAILURE;
	}
	...
	reply = amqp_get_rpc_reply(**conn);
	if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot declare queue - %s\n", amqp_error_string2(reply.library_error));
		return RABBITMQ_FAILURE;
	}

	*queuename = amqp_bytes_malloc_dup(rq->queue);
	if (queuename->bytes == NULL) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Out of memory while copying queue name\n");
		return RABBITMQ_FAILURE;
	}
				...
				char *username, char *password, char *queue_name, char *exchange, int rabbitmq_deliveries)
{
	...
	if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot bind queue - %s\n", amqp_error_string2(reply.library_error));
		return RABBITMQ_FAILURE;
	}
	...
	reply = amqp_get_rpc_reply(*conn);
	if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot set basic qos - %s\n", amqp_error_string2(reply.library_error));
		return RABBITMQ_FAILURE;
	}
	...
	reply = amqp_get_rpc_reply(*conn);
	if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot set basic consume - %s\n", amqp_error_string2(reply.library_error));
		return RABBITMQ_FAILURE;
	}

	pmesg(LOG_DEBUG, __FILE__, __LINE__, "Consume connection to RabbitMQ broker on queue %s, ip_address %s and port %s\n", queue_name, hostname, port);
...
int rabbitmq_publish_connection(amqp_connection_state_t * conn, amqp_channel_t channel, char *hostname, char *port, char *username, char *password, char *queue_name)
{
	...
	pmesg(LOG_DEBUG, __FILE__, __LINE__, "Publish connection to RabbitMQ broker on queue %s, ip_address %s and port %s\n", queue_name, hostname, port);
...
int close_rabbitmq_connection(amqp_connection_state_t conn, amqp_channel_t channel)
{
	if (conn != NULL && (int) channel > 0) {
		...
		if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot close channel - %s\n", amqp_error_string2(reply.library_error));
		...
		if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot close connection - %s\n", amqp_error_string2(reply.library_error));
			return RABBITMQ_FAILURE;
		}

		int status = amqp_destroy_connection(conn);
		if (status != AMQP_STATUS_OK) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot destroy connection\n");
			return RABBITMQ_FAILURE;
		}

		return RABBITMQ_SUCCESS;
	} else {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Cannot close RabbitMQ connection\n");
		return RABBITMQ_FAILURE;
	}
}
