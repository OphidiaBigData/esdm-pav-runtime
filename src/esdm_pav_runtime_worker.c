...
int process_message(amqp_envelope_t full_message)
{
	...
	if (exec_pid == 0) {
		int neededSize = snprintf(NULL, 0, "%s -n %s -outfile-pattern=%s -errfile-pattern=%s %s %s", launcher, ncores, log_string, log_string, framework_path, submission_string);
		char *exec_string = (char *) malloc(neededSize + 1);
		snprintf(exec_string, neededSize + 1, "%s -n %s -outfile-pattern=%s -errfile-pattern=%s %s %s", launcher, ncores, log_string, log_string, framework_path, submission_string);
